<template>
    <div class="slide__cont--two">
        ...
        <StudyResponseOption v-for="option in options" :key="option.value" :text="option.text" :value="option.value"
            :index="props.index" :id="props.id" :modelValue="currentResponseType"
            @update:modelValue="updateResponseType" :disabled="isDisabled" />

        <div class="aside__container aside__selection">
            <label for="demographic_required_checkbox"
                class="aside__label aside__label--headline font-normal font-medium">
                Required
            </label>
            <input type="checkbox" id="demographic_required_checkbox" class="aside__checkbox" v-model="requiredModel"
                @change="updateRequired()" :disabled="isDisabled" />
        </div>

        <div class="aside__container aside__container--borderless">
            <button v-if="!isDisabled" class="aside__button aside__button--delete font-semi font-small"
...
const emit = defineEmits(['update:modelValue', 'delete', 'toggle']);
