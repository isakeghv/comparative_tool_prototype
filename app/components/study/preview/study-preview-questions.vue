	...
	<section class="question__cont">
		<div class="question__cont--top">
			...
			<div class="question__panel" :class="{'question__panel--display': selectedSource, 'question__panel--hidden': !selectedSource}">
				<button class="question__button" @click="selectedSource = null" v-if="selectedSource">Exit</button>
		...
		<div v-if="currentQuestion.responseType === 'drop'" class="drop__row">
			<ArtifactDropBox @mouseover="box_mouseover()" @mouseleave="box_mouseleave()"
				...
				v-if="responseType === 'drop'" v-for="(box, index) in currentQuestion.drop.dropBox.filter(box => box !== '')" :key="index"
...
<script setup>
import ArtifactPreview from '~/components/participant/artifact/artifact-preview.vue';
import ArtifactDropLinear from '~/components/participant/artifact/artifact-drop-linear.vue';
import ArtifactDropBox from '~/components/participant/artifact/artifact-drop-box.vue';
import { linear_moving, linear_artifactIsOver, linear_artifactOver, linear_drop, linear_orderUp, linear_orderDown, linear_insertAt } from '~/components/participant/linearFunctionality';
import { box_moving, box_mouseover, box_drop, box_mouseleave } from '~/components/participant/dropboxFunctionality';
...
//deleting, item from study-object, so that when the preview is re-opened, answer are not saved
if (currentQuestion.value?.id) delete participantAnswer[currentQuestion.value?.id];
